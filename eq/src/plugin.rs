...
use clap_clap::{
    ...
    ffi::{
        ...
        CLAP_INVALID_ID, CLAP_PARAM_REQUIRES_PROCESS, CLAP_PLUGIN_FACTORY_ID,
...
impl SharedState {
    ...
    fn take_pending_param_notifications(&self, dest: &mut [u32; 5]) {
        for i in 0..5 {
            dest[i] = self.pending_param_notifications[i].swap(0, Ordering::AcqRel);
        }
    }
    ...
    fn requeue_pending_param_notifications(&self, bits: &[u32; 5]) {
        for i in 0..5 {
            if bits[i] != 0 {
                self.pending_param_notifications[i].fetch_or(bits[i], Ordering::AcqRel);
            }
        }
    }
