...
const PLUGIN_ID: &[u8] = b"rs.maolan.chorus\0";
const PLUGIN_NAME: &[u8] = b"Maolan Chorus\0";
const PLUGIN_URL: &[u8] = b"\0";
const PLUGIN_VENDOR: &[u8] = b"Maolan\0";
const PLUGIN_VERSION: &[u8] = b"0.1.0\0";
const PLUGIN_DESCRIPTION: &[u8] = b"Stereo chorus based on Mire Chorus\0";
...
fn parse_param_text(id: ParamId, text: &str) -> Option<f64> {
    match id {
        ...
        ParamId::Voices => text.trim().parse().ok(),
    }
}
...
impl AudioProcessor {
    ...
    fn process(&mut self, shared: &SharedState, process: &mut Process) -> clap_process_status {
        crate::common::apply_param_events(shared, &process.in_events(), sanitize_param_value);
        {
            let mut out_events = process.out_events();
            crate::common::emit_pending_param_events_to_host(shared, &mut out_events);
        }
...
impl StateHooks<SharedState> for PluginState {
    fn save(shared: &SharedState) -> Option<Vec<u8>> {
        PluginState::from_runtime(&shared.params)
            .to_bytes(crate::chorus::state::STATE_HEADER_PREFIX)
            .ok()
    }

    fn load(shared: &SharedState, bytes: &[u8]) -> bool {
        let Ok(state) = PluginState::from_bytes(bytes, crate::chorus::state::STATE_HEADER_PREFIX)
        else {
            return false;
        };
