...
use crate::common::{apply_param_events, bus, emit_pending_param_events_to_host, fft};
use crate::deesser::{
...
impl StateHooks<SharedState> for PluginState {
    fn save(shared: &SharedState) -> Option<Vec<u8>> {
        PluginState::from_runtime(&shared.params)
            .to_bytes(crate::deesser::state::STATE_HEADER_PREFIX)
            .ok()
    }

    fn load(shared: &SharedState, bytes: &[u8]) -> bool {
        let Ok(state) = PluginState::from_bytes(bytes, crate::deesser::state::STATE_HEADER_PREFIX)
        else {
            return false;
        };
...
clap_create_fn!(
    ...
    GuiBridge,
    PORTS,
    bus bus::PluginSharedData::new(bus::PluginType::Deesser).with_fft(bus::FftData::default())
);
