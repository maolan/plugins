...
use crate::deesser::{
    dsp::{DeEsser, DeEsserParams},
    gui::GuiBridge,
    params::{PARAMS, ParamId, ParamStore, sanitize_param_value},
    state::PluginState,
};

const PLUGIN_ID: &[u8] = b"rs.maolan.deesser\0";
const PLUGIN_NAME: &[u8] = b"Maolan DeEsser\0";
const PLUGIN_VENDOR: &[u8] = b"Maolan\0";
const PLUGIN_URL: &[u8] = b"\0";
const PLUGIN_VERSION: &[u8] = b"0.1.0\0";
const PLUGIN_DESCRIPTION: &[u8] = b"Sibilance reduction processor\0";
    ...
    fn process(&mut self, shared: &SharedState, process: &mut Process) -> clap_process_status {
        ...
        if inputs_count >= 2 && outputs_count >= 2 {
            ...
            self.dsp.process_stereo(
                ...
                &DeEsserParams {
                    intensity: shared.params.get(ParamId::Intensity),
                    sharpness: shared.params.get(ParamId::Sharpness),
                    depth: shared.params.get(ParamId::Depth),
                    filter: shared.params.get(ParamId::Filter),
                    monitor: shared.params.get_bool(ParamId::Monitor),
                },
            );
...
impl PluginInstance {
    fn new(host: *const clap_host) -> Self {
        ...
        let mut bus_data = bus::PluginSharedData::new(bus::PluginType::Deesser).with_fft(bus::FftData::default());
...
unsafe extern "C-unwind" fn ext_gui_is_api_supported(
    ...
    crate::deesser::gui::is_api_supported(api, is_floating)
}
...
unsafe extern "C-unwind" fn ext_gui_get_preferred_api(
    ...
    let preferred = crate::deesser::gui::preferred_api();
    unsafe {
...
unsafe extern "C-unwind" fn ext_gui_get_size(
    ...
    unsafe {
        *width = crate::deesser::gui::EDITOR_WIDTH;
        *height = crate::deesser::gui::EDITOR_HEIGHT;
    }
    true
}
...
unsafe extern "C-unwind" fn ext_gui_set_parent(
    ...
    let parent = if api == CLAP_WINDOW_API_X11 {
        ...
        {
            crate::deesser::gui::ParentWindowHandle::X11(unsafe { window.clap_window__.x11 })
        }
        ...
        #[cfg(target_os = "macos")]
        {
            crate::deesser::gui::ParentWindowHandle::Cocoa(unsafe { window.clap_window__.cocoa })
        }
        ...
        {
            crate::deesser::gui::ParentWindowHandle::Win32(unsafe { window.clap_window__.win32 })
        }
