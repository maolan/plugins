...
use maolan_clap::{
    ...
    ffi::{
        ...
        clap_host, clap_host_params, clap_host_state, clap_id, clap_istream, clap_ostream,
...
use crate::flanger::{
    ...
    gui::GuiBridge,
    params::{
        FRACTION_NAMES, LFO_TYPE_NAMES, LFO2_TYPE_NAMES, PARAMS, PERIOD_NAMES, ParamId, ParamStore,
...
pub struct SharedState {
    ...
    pending_gesture_begin: AtomicU32,
    pending_gesture_end: AtomicU32,
    active_local_gestures: AtomicU32,
...
impl Default for SharedState {
    fn default() -> Self {
        Self {
            ...
            pending_gesture_begin: AtomicU32::new(0),
            pending_gesture_end: AtomicU32::new(0),
            active_local_gestures: AtomicU32::new(0),
...
unsafe extern "C-unwind" fn ext_gui_is_api_supported(
    ...
    crate::flanger::gui::is_api_supported(api, is_floating)
}
...
unsafe extern "C-unwind" fn ext_gui_get_preferred_api(
    ...
    let preferred = crate::flanger::gui::preferred_api();
    unsafe {
...
unsafe extern "C-unwind" fn ext_gui_get_size(
    ...
    unsafe {
        *width = crate::flanger::gui::EDITOR_WIDTH;
        *height = crate::flanger::gui::EDITOR_HEIGHT;
    }
    true
}
...
unsafe extern "C-unwind" fn ext_gui_set_parent(
    ...
    let parent = if api == CLAP_WINDOW_API_X11 {
        ...
        {
            crate::flanger::gui::ParentWindowHandle::X11(unsafe { window.clap_window__.x11 })
        }
        ...
        {
            crate::flanger::gui::ParentWindowHandle::Win32(unsafe { window.clap_window__.win32 })
        }
        ...
        #[cfg(target_os = "macos")]
        {
            crate::flanger::gui::ParentWindowHandle::Cocoa(unsafe { window.clap_window__.cocoa })
        }
...
static AUDIO_PORTS_EXT_REF: clap_plugin_audio_ports = AUDIO_PORTS_EXT;
static PARAMS_EXT_REF: clap_plugin_params = PARAMS_EXT;
static STATE_EXT_REF: clap_plugin_state = STATE_EXT;
static TAIL_EXT_REF: clap_plugin_tail = TAIL_EXT;
...
unsafe extern "C-unwind" fn plugin_get_extension(
    ...
    if id == CLAP_EXT_AUDIO_PORTS {
        &raw const AUDIO_PORTS_EXT_REF as *const _ as *const c_void
    } else if id == CLAP_EXT_PARAMS {
        &raw const PARAMS_EXT_REF as *const _ as *const c_void
    } else if id == CLAP_EXT_STATE {
        &raw const STATE_EXT_REF as *const _ as *const c_void
    } else if id == CLAP_EXT_TAIL {
        &raw const TAIL_EXT_REF as *const _ as *const c_void
