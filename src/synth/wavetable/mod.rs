...
impl Wavetable {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        if wave_size < 2 || wave_size > MAX_WTABLE_SIZE {
            return None;
        }
        if wave_count < 1 || wave_count > MAX_SUBTABLES {
            return None;
        }
        ...
        if is_int16 {
            ...
            let scale = if is_full16 { 1.0 / 32768.0 } else { 1.0 / 16384.0 };
            for t in 0..wave_count {
                let mut frame = vec![0.0f32; wave_size];
                for s in 0..wave_size {
    ...
    fn build_mipmaps(&mut self) {
        ...
        while current_size > 2 {
            ...
            for frame_idx in 0..self.n_tables {
                let mut new_frame = vec![0.0f32; current_size];
                let prev_frame = &prev[frame_idx];
