...
impl Tuning {
    ...
    pub fn from_scl(content: &str) -> Result<Self, String> {
        ...
        for line in lines {
            ...
            if expected_count == 0 {
                expected_count = line.parse().map_err(|e| format!("bad degree count: {}", e))?;
                degrees.push(0.0);
                continue;
            }
            let cents = if line.contains('/') {
                ...
                let num: f32 = parts[0].parse().map_err(|e| format!("bad ratio num: {}", e))?;
                let den: f32 = parts[1].parse().map_err(|e| format!("bad ratio den: {}", e))?;
                1200.0 * (num / den).log2()
