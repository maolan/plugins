...
const PLUGIN_ID: &[u8] = b"rs.maolan.monitoring\0";
const PLUGIN_NAME: &[u8] = b"Maolan Monitoring\0";
const PLUGIN_VENDOR: &[u8] = b"Maolan\0";
const PLUGIN_URL: &[u8] = b"\0";
const PLUGIN_VERSION: &[u8] = b"0.1.0\0";
const PLUGIN_DESCRIPTION: &[u8] = b"Rust CLAP Monitoring based on Monitoring\0";
    ...
    fn process(&mut self, shared: &SharedState, process: &mut Process) -> clap_process_status {
        ...
        if inputs_count >= 2 && outputs_count >= 2 {
            ...
            self.dsp.process_stereo(
                ...
                shared.params.get(ParamId::Mode) as u32,
                shared._sample_rate() as f64,
            );
...
unsafe extern "C-unwind" fn ext_gui_is_api_supported(
    ...
    crate::monitoring::gui::is_api_supported(api, is_floating)
}
...
unsafe extern "C-unwind" fn ext_gui_get_preferred_api(
    ...
    let preferred = crate::monitoring::gui::preferred_api();
    unsafe {
...
unsafe extern "C-unwind" fn ext_gui_get_size(
    ...
    unsafe {
        *width = crate::monitoring::gui::EDITOR_WIDTH;
        *height = crate::monitoring::gui::EDITOR_HEIGHT;
    }
    true
}
...
unsafe extern "C-unwind" fn ext_gui_set_parent(
    ...
    let parent = if api == CLAP_WINDOW_API_X11 {
        ...
        {
            crate::monitoring::gui::ParentWindowHandle::X11(unsafe { window.clap_window__.x11 })
        }
        ...
        #[cfg(target_os = "macos")]
        {
            crate::monitoring::gui::ParentWindowHandle::Cocoa(unsafe { window.clap_window__.cocoa })
        }
        ...
        {
            crate::monitoring::gui::ParentWindowHandle::Win32(unsafe { window.clap_window__.win32 })
        }
