...
const PLUGIN_ID: &[u8] = b"rs.maolan.vocoder\0";
const PLUGIN_NAME: &[u8] = b"Maolan Vocoder\0";
const PLUGIN_VENDOR: &[u8] = b"Maolan\0";
const PLUGIN_URL: &[u8] = b"\0";
const PLUGIN_VERSION: &[u8] = b"0.1.0\0";
const PLUGIN_DESCRIPTION: &[u8] = b"Stereo vocoder based on Mire Vocoder\0";
...
impl AudioProcessor {
    ...
    fn process(&mut self, shared: &SharedState, process: &mut Process) -> clap_process_status {
        ...
        let params = crate::vocoder::dsp::VocoderParams {
            spectral_shift: shared.params.get(ParamId::SpectralShift),
        ...
        if inputs_count >= 2 && outputs_count >= 2 {
            ...
            self.dsp
                .process_mono(&mut self.temp_left[..frames], &params);
...
fn parse_param_text(id: ParamId, text: &str) -> Option<f64> {
    match id {
        ParamId::SpectralShift => text.trim().parse().ok(),
        ParamId::DryWet => {
...
impl StateHooks<SharedState> for PluginState {
    fn save(shared: &SharedState) -> Option<Vec<u8>> {
        PluginState::from_runtime(&shared.params)
            .to_bytes(crate::vocoder::state::STATE_HEADER_PREFIX)
            .ok()
    }

    fn load(shared: &SharedState, bytes: &[u8]) -> bool {
        let Ok(state) = PluginState::from_bytes(bytes, crate::vocoder::state::STATE_HEADER_PREFIX)
        else {
            return false;
        };
