...
const PLUGIN_ID: &[u8] = b"rs.maolan.formant\0";
const PLUGIN_NAME: &[u8] = b"Maolan Formant\0";
const PLUGIN_VENDOR: &[u8] = b"Maolan\0";
const PLUGIN_URL: &[u8] = b"\0";
const PLUGIN_VERSION: &[u8] = b"0.1.0\0";
const PLUGIN_DESCRIPTION: &[u8] = b"Stereo formant filter based on Mire Formant\0";
...
impl AudioProcessor {
    ...
    fn process(&mut self, shared: &SharedState, process: &mut Process) -> clap_process_status {
        ...
        let params = FormantParams {
            vowel: shared.params.get(ParamId::Vowel),
...
fn param_text(id: ParamId, value: f64) -> String {
    match id {
        ParamId::Vowel => format!("{value:.2}"),
        ParamId::Sharpness => format!("{value:.1}"),
...
fn parse_param_text(id: ParamId, text: &str) -> Option<f64> {
    match id {
        ...
        ParamId::Vowel | ParamId::Sharpness => text.trim().parse().ok(),
    }
}
...
impl StateHooks<SharedState> for PluginState {
    fn save(shared: &SharedState) -> Option<Vec<u8>> {
        PluginState::from_runtime(&shared.params)
            .to_bytes(crate::formant::state::STATE_HEADER_PREFIX)
            .ok()
    }

    fn load(shared: &SharedState, bytes: &[u8]) -> bool {
        let Ok(state) = PluginState::from_bytes(bytes, crate::formant::state::STATE_HEADER_PREFIX)
        else {
            return false;
        };
