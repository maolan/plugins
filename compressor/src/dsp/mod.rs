...
impl BandCompressor {
    ...
    fn gain_db(&mut self, sidechain: f32, sc_mode: u32, mode: u32) -> f32 {
        let env_db = if sc_mode == 1 {
            ...
            self.rms_env += self.attack_coef * (squared - self.rms_env);
            if squared > self.rms_env {
                self.rms_env = squared;
            }
            gain_to_db(self.rms_env.sqrt().max(1.0e-10))
        } else {
            let abs_in = sidechain.abs();
            if abs_in > self.peak_env {
                self.peak_env = abs_in;
            } else {
...
impl Compressor {
    ...
    pub fn process_stereo(&mut self, left: &mut [f32], right: &mut [f32]) {
        ...
        for i in 0..frames {
            ...
            let (aud_l, aud_r) = self.audio_split.split4(delayed_l, delayed_r);
